...
int16_t ifm_data[IFM_ROWS * IFM_COLS];
int16_t kernel_data[KERNEL_ROWS * KERNEL_COLS];
int16_t ofm_data[OFM_ROWS * OFM_COLS]; // Accelerator writes here
int16_t ofm_expected_data[OFM_ROWS * OFM_COLS]; // For software reference

// Function to initialize data (simple pattern)
void initialize_data() {
    printf("Initializing IFM and Kernel data...\n");
    for (int i = 0; i < IFM_ROWS * IFM_COLS; ++i) {
        ifm_data[i] = (i % 100) + 1; // Simple pattern, 1 to 100
    }
    for (int i = 0; i < KERNEL_ROWS * KERNEL_COLS; ++i) {
        kernel_data[i] = (i % 5) + 1; // Simple pattern, 1 to 5
    }
...
// Simple software 'same' convolution for verification (fixed point aspects ignored for simplicity here)
// This is a conceptual reference; a true fixed-point reference would be more complex.
void software_convolution_reference() {
    printf("Calculating software reference convolution...\n");
    ...
    for (int orow = 0; orow < OFM_ROWS; ++orow) {
        for (int ocol = 0; ocol < OFM_COLS; ++ocol) {
            int32_t acc = 0; // Use 32-bit accumulator for intermediate products
            for (int krow = 0; krow < KERNEL_ROWS; ++krow) {
                for (int kcol = 0; kcol < KERNEL_COLS; ++kcol) {
                    ...
                    if (ifm_row_eff >= 0 && ifm_row_eff < IFM_ROWS &&
                        ...
                        acc += (int32_t)ifm_data[ifm_row_eff * IFM_COLS + ifm_col_eff] *
                               (int32_t)kernel_data[krow * KERNEL_COLS + kcol];
                    }
                    // Padded values are 0, so they don't add to acc
                }
            }
            // In a real fixed-point scenario, right-shift (F_BITS) and saturation/clipping would happen here.
            // For this basic test, we'll just assign the sum.
            // This won't match the hardware exactly if F_BITS > 0, but tests data movement and control flow.
            ofm_expected_data[orow * OFM_COLS + ocol] = (int16_t)(acc >> 8); // Example: assume 8 fractional bits
        }
    }
    printf("Software reference calculation complete.\n");
}
...
int main(void) {
    ...
    initialize_data();
    // software_convolution_reference(); // Calculate reference for later comparison
    ...
    // Note: RoCC might stay IDLE after just ADDR config, and only go to CONFIG_READY after PARAMS.
    ...
    printf("--- Verification (Conceptual) ---\n");
    printf("Sample OFM data (from accelerator's DMA write to memory):\n");
    for (int i = 0; i < 10 && i < OFM_ROWS * OFM_COLS; ++i) { // Print more samples
        printf("OFM_MEM[%d] = %d\n", i, ofm_data[i]);
    }
    // Conceptual verification against a software model would go here.
    // software_convolution_reference(); // Calculate expected
    // int errors = 0;
    // for (int i = 0; i < OFM_ROWS * OFM_COLS; ++i) {
    //     if (ofm_data[i] != ofm_expected_data[i]) {
    //         if(errors < 10) printf("Mismatch at OFM[%d]: Expected %d, Got %d\n", i, ofm_expected_data[i], ofm_data[i]);
    //         errors++;
    //     }
    // }
    // if (errors == 0) {
    //     printf("OFM data matches software reference (conceptual check - requires accurate fixed-point ref).\n");
    // } else {
    //     printf("%d mismatches in OFM data compared to software reference.\n", errors);
    // }

    printf("CNN Accelerator RoCC Test finished successfully.\n");
    ...
    if (current_status == STATUS_IDLE || current_status == STATUS_DMA_OFM_STORE_DONE) {
         printf("Test PASSED: Operations completed and final status (%lu) is reasonable.\n", current_status);
    } else {
